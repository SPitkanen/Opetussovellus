CREATE TABLE users (id SERIAL PRIMARY KEY, name TEXT UNIQUE, password TEXT UNIQUE, role TEXT);
CREATE TABLE courses (id SERIAL PRIMARY KEY, name TEXT UNIQUE, teacher_id INTEGER REFERENCES users(id), visible INTEGER);
...
CREATE TABLE choices (id SERIAL PRIMARY KEY, task_id INTEGER REFERENCES tasks(id), text TEXT, correct INTEGER, typ INTEGER, visible INTEGER);
